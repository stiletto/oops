...
logfile_t	*curr;
    ...
    curr = logfiles;
    while(curr) {
	if ( curr->path ) my_xlog(LOG_NOTICE|LOG_DBG|LOG_INFORM, "Reopen %s\n", curr->path);
	if ( curr->file ) fclose(curr->file);
...
logfile_t *curr;
	...
	if ( curr->path ) {
		...
		my_xlog(LOG_SEVERE, "custom_log:fopen(%s): %s\n",
...
char	*p = config;
	...
	if ( current_config ) {
	    ...
	    verb_printf("No current logfile\n");
	}
	return(MOD_CODE_OK);
    }
